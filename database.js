...
class InMemoryDB {
    ...
    async addBattlePassXp(username, xpAmount) { const user = await this.findUserByUsername(username); if (!user) return null; const battlePass = user.battlePass || { season: 1, level: 0, xp: 0, totalXp: 0, claimedRewards: [], quests: [], questsDate: null, currentTier: 1, tierXp: 0, maxTierXp: 1000, freeRewardsClaimed: [], premiumRewardsClaimed: [], isPremium: false, premiumRequestedAt: null }; battlePass.xp += xpAmount; battlePass.totalXp += xpAmount; battlePass.tierXp += xpAmount; let levelsGained = 0; let tiersGained = 0; const xpPerLevel = 1000; while (battlePass.xp >= xpPerLevel) { battlePass.xp -= xpPerLevel; battlePass.level += 1; levelsGained += 1; } while (battlePass.tierXp >= battlePass.maxTierXp) { battlePass.tierXp -= battlePass.maxTierXp; battlePass.currentTier += 1; tiersGained += 1; battlePass.maxTierXp = 1000 + (battlePass.currentTier - 1) * 50; } await this.updateUser(username, { battlePass }); return { battlePass, levelsGained, tiersGained }; }
